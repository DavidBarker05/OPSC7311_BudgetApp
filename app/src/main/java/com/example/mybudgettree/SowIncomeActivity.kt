...
class SowIncomeActivity : AppCompatActivity() {

    companion object {
        private const val TAG = "SowIncomeActivity"
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_sow_income)
        ...
        findViewById<MaterialButton>(R.id.btnSaveExpense).setOnClickListener { saveIncome() }
        bindDate()
        bindTimes()
            ...
            category == null -> Toast.makeText(this, R.string.select_category, Toast.LENGTH_SHORT).show()
                ...
                lifecycleScope.launch {
                    ...
                    if (result.wasSuccessful) {
                        Log.i(TAG, "Saved income '$title' for category '${category.categoryName}'")
                        Toast.makeText(this@SowIncomeActivity, R.string.income_saved, Toast.LENGTH_SHORT).show()
                        finish()
                    } else {
                        Log.w(TAG, "Failed to save income '$title': ${result.errMsg}")
                        Toast.makeText(
                            this@SowIncomeActivity,
    ...
    private fun launchCamera() {
        val file = File(cacheDir, "income_${System.currentTimeMillis()}.jpg")
        cameraFile = file
    ...
    private fun saveReceiptFromUri(uri: Uri) {
        ...
        lifecycleScope.launch {
            ...
            receiptPath = app.imageStorageSystem.saveImage(bytes, "income_${System.currentTimeMillis()}.jpg")
            showReceiptPreview()
        }
    }
